    ...
    <table>
      ...
      <tbody>
        <template v-for="(vault, index) in vaults" :key="vault.id">
          <tr @click="toggleDetail(vault.id)">
            <td>{{ index + 1 }}
            ...
            <td class="desktop" style="text-align: right">{{ vault.backedPercent }}</td>
          </tr>
            <tr v-if="expandedVaultId === vault.id" class="mobile-detail-btn">
                <td style="border-top: 0px; padding: 0px 0px 15px 0px" colspan="3">
                <LearnMoreButton buttonColor="rgb(48, 48, 48)" buttonText="Learn more" textColor="#fff" />
                </td>
            </tr>
        </template>
      </tbody>
...
function toggleDetail(id) {
  expandedVaultId.value = expandedVaultId.value === id ? null : id;
            ...
            // <div v-if="expandedVaultId === vault.id" class="mobile-detail-btn">
            //     <LearnMoreButton buttonColor="#FFF" buttonText="Learn more" />
            // </div>
</script>
